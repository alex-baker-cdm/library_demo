...
interface PlacingOnHoldPolicy extends Function3<Book, Patron, HoldDuration, Either<Rejection, Allowance>> {

    PlacingOnHoldPolicy onlyResearcherPatronsCanHoldRestrictedBooksPolicy = (Book toHold, Patron patron, HoldDuration holdDuration) -> {
        if (toHold.getBookType() == io.pillopl.library.catalogue.BookType.Restricted && patron.isRegular()) {
            return left(Rejection.withReason("Regular patrons cannot hold restricted books"));
        }
        return right(new Allowance());
    };

    PlacingOnHoldPolicy overdueCheckoutsRejectionPolicy = (Book toHold, Patron patron, HoldDuration holdDuration) -> {
        if (patron.overdueCheckoutsAt(toHold.getCurrentBranch()) >= OverdueCheckouts.MAX_COUNT_OF_OVERDUE_RESOURCES) {
            return left(Rejection.withReason("cannot place on hold when there are overdue checkouts"));
        }
        return right(new Allowance());
    };

    PlacingOnHoldPolicy regularPatronMaximumNumberOfHoldsPolicy = (Book toHold, Patron patron, HoldDuration holdDuration) -> {
    ...
    PlacingOnHoldPolicy onlyResearcherPatronsCanPlaceOpenEndedHolds = (Book toHold, Patron patron, HoldDuration holdDuration) -> {
