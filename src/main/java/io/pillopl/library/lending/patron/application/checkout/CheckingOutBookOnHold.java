...
public class CheckingOutBookOnHold {
    ...
    public Try<Result> checkOut(CheckOutBookCommand command) {
        return Try.of(() -> {
            Book book = find(command.getBookId(), command.getPatronId());
            Patron patron = find(command.getPatronId());
            Either<BookCheckingOutFailed, BookCheckedOut> result = patron.checkOut(book, command.getCheckoutDuration());
